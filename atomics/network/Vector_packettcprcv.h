#if !defined Vector_packettcprcv_h
#define Vector_packettcprcv_h
 ...
 * packettcprcv has a few details with parameters (string params and params that depend on index) which makes it hard  to use VectoriaModel
 */
class Vector_packettcprcv: public Simulator {
protected:
	Coupling* D0;
	...
	packettcprcv* createScalarInstance(char* scalarModelName){
		return new packettcprcv(scalarModelName);
	}

public:
	Vector_packettcprcv(const char *n): Simulator(n) {};
	virtual ~Vector_packettcprcv() { }
