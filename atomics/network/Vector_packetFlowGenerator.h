#if !defined Vector_packetFlowGenerator_h
#define Vector_packetFlowGenerator_h
 ...
 * packetFlowGenerator has a string parameters  which makes it hard  to use VectoriaModel template
 */
class Vector_packetFlowGenerator: public Simulator {
protected:
	Coupling* D0;
	...
	PacketFlowGenerator* createScalarInstance(char* scalarModelName){
		return new PacketFlowGenerator(scalarModelName);
	}

public:
	Vector_packetFlowGenerator(const char *n): Simulator(n) {};
	virtual ~Vector_packetFlowGenerator() { }
	...
	void init(double t,...) {
		...
		std::cout << "scalarInstances" << scalarInstances <<std::endl;
