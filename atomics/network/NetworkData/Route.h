...
public:
	...
	Route(const std::deque<Node>& routePath) : routePath(routePath) {
	}
	...
	void printInfo(int level) const{
		printLog(level, "\t Route has %u jumps: ", this->jumpCount());
		for(Node node : routePath){
			printLog(level, " {nodeName='%s'; outPort=%d},", node.nodeName.data(), node.outgoingPort);
