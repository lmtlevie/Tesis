#include "qss3_advanced.h"

void qss3_advanced::init(double t,...) {
	...
	sprintf(buff2,"t:%f\n",t);
	strcat(buff,buff2);
	sprintf(buff2,"x:0\t%f\t%f\t%f\t%f\n",X[0],X[1],X[2],X[3]);
	strcat(buff,buff2);
	sprintf(buff2,"q:0\t%f\t%f\t%f\t%f\n",dQ,q[0],q[1],q[2]);
	strcat(buff,buff2);
...
double qss3_advanced::ta(double t) {
	//This function return a double.
	return sigma;
}

void qss3_advanced::dint(double t) {
...
void qss3_advanced::dext(Event x, double t) {
	...
	if (sigma >0){
		...
		char buf[1024];
		sprintf(buf,"t:%f\nx:0\t%f\t%f\t%f\t%f\nq:0\t%f\t%f\t%f\t%f\n",t,X[0],X[1],X[2],X[3],dQ,q[0],q[1],q[2]);
		PDFileWrite(foutput,buf,strlen(buf));
	}
}

Event qss3_advanced::lambda(double t) {
