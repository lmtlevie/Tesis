#include "qss2_advanced.h"

void qss2_advanced::init(double t,...) {
	...
	sprintf(buff2,"t:%f\n",t);
	strcat(buff,buff2);
	sprintf(buff2,"x:0\t%f\t%f\t%f\n",X[0],X[1],X[2]);
	strcat(buff,buff2);
	sprintf(buff2,"q:0\t%f\t%f\t%f\n",dQ,q[0],q[1]);
	strcat(buff,buff2);
...
double qss2_advanced::ta(double t) {
	//This function return a double.
	return sigma;
}

void qss2_advanced::dint(double t) {
...
void qss2_advanced::dext(Event x, double t) {
	...
	if (sigma >0){
		// t:timestamp
		// x:0 <a0> <a1> <a2>
		// q:0 <QUANTUM ACTUAL> <a0> <a1>
	
		char buf[1024];
		sprintf(buf,"t:%f\nx:0\t%f\t%f\t%f\nq:0\t%f\t%f\t%f\n",t,X[0],X[1],X[2],dQ,q[0],q[1]);
		PDFileWrite(foutput,buf,strlen(buf));
	}
}

Event qss2_advanced::lambda(double t) {
