...
//////////////////////////////// STDEVS
mySTDEVS = new STDEVS(STDEVS_DEFAULT_SEED);
...
BW = (double)readDefaultParameterValue<double>(fvar);
printLog("[%g] PacketGen: BW: %g bits/seg \n",t,BW);

fvar = va_arg(parameters,char*);
AvgPktSize = (double)readDefaultParameterValue<double>(fvar);
printLog("[%g] PacketGen: Average Packet Size: %g bits \n",t,AvgPktSize);

fvar = va_arg(parameters,char*);
MinPktSize = (double)readDefaultParameterValue<double>(fvar);
printLog("[%g] PacketGen: Min Packet Size: %g bits \n",t,MinPktSize);

fvar = va_arg(parameters,char*);
MaxPktSize = (double)readDefaultParameterValue<double>(fvar);
...
myLoggerToScilab->initSignals(std::vector<std::string>{"pktlength","intergen"});
...
p->calcLength();
double templength=(double)p->getLength();
