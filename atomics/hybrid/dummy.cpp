...
//    mySTDEVS = new STDEVS(_STDEVS_DEFAULT_SEED);
      mySTDEVS = new STDEVS(STDEVS_UNIQUE_SEED);
...
Period = (double)readDefaultParameterValue<double>(fvar);
printLog("[%g] PacketGen: Period: %g \n",t,Period);

fvar = va_arg(parameters,char*);
Psize = (double)readDefaultParameterValue<double>(fvar);
printLog("[%g] PacketGen: Packet Size by Parameter: %g bits \n",t,Psize);

fvar = va_arg(parameters,char*);
StartTime = (double)readDefaultParameterValue<double>(fvar);
...
myprotoStack.push_back(std::make_shared<protocol>(myprotocol));
