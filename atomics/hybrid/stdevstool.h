...
#include <sys/time.h>
#include <cstdlib> // std:srand to set seed
...
#define STDEVS_REPRODUCIBLE_SEED 1  // Random reproducible generation. reproducibility of results
#define STDEVS_UNIQUE_SEED 0  // Random unique generation
...
template<typename T> T readDefaultParameterValue(const std::string& paramName);
...
public:
 ...
 // Unless for specific needs, always use this instance which will point to one of the previous based on the configuration or to a custom seeded.
 ...
 STDEVS(int randSeed);
  ...
  ~STDEVS() {printLog(10, "STDEVS Destroyed \n");};
	...
	double normal(double mu, double sigma) { return gsl_ran_gaussian(this -> r, sigma) + mu;}
	double pareto(double order, double minValue) {	return gsl_ran_pareto(this -> r, order, minValue); }
	double lognormal(double zeta, double sigma) {	return gsl_ran_lognormal(this->r, zeta, sigma); }
	double gamma(double shape, double scale) {	return gsl_ran_gamma(this->r, shape, scale); }
