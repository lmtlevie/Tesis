...
latency = (double)readDefaultParameterValue<double>(fvar);
printLog("[%g] Channel: Latency: %g ms. \n",t,latency);

fvar = va_arg(parameters,char*);
bandwidth = (double)readDefaultParameterValue<double>(fvar);
...
myLoggerToScilab->initSignals(std::vector<std::string>{"arrived","departed","dropped","queuesize","proptime"});
		...
		case SENDING:
			if ((int)myPacketChannel.size()>0){ // Channel is not empty
				...
				myLoggerToScilab->logSignal(t,proptime,"proptime");
  			 	...
  			 	currentInFlightSize-=p->getLength();
