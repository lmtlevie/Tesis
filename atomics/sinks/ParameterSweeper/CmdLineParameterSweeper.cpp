...
int CmdLineParameterSweeper::getCurrentExperimentNumber(){
	int value;
	if(getCmdLineParam<int>(simuRunVariableName, value))
		return value;
...
int CmdLineParameterSweeper::getTotalNumberOfExperiments(){
	...
	try { // handle exception if not defined
		parameterValues = getCmdLineParam<std::vector<double>>(parameterValuesVariableName);
		parameterNames = getCmdLineParam<std::vector<std::string>>(parameterNamesVariableName);
	} catch (...) {
		printLog(LOG_LEVEL_ERROR, "WARNING: parameter '%s' and/or '%s' was not defined (is it intentional?). Setting both to empty lists. \n", parameterValuesVariableName.data(), parameterNamesVariableName.data());
...
void CmdLineParameterSweeper::UpdateParameters(){
	...
	try { // handle exception if not defined
			parameterValues = getCmdLineParam<std::vector<std::string>>(parameterValuesVariableName);
			parameterNames = getCmdLineParam<std::vector<std::string>>(parameterNamesVariableName);
	} catch (...) {
		printLog(LOG_LEVEL_ERROR, "WARNING: parameter '%s' and/or '%s' was not defined (is it intentional?). Setting both to empty lists. \n", parameterValuesVariableName.data(), parameterNamesVariableName.data());
		parameterValues.clear();
	...
	if(parameterNames.size() == 0) return; // nothing else to be done
	if(parameterNames.size() > 1)
		printLog(LOG_LEVEL_ERROR, "CmdLineParameterSweeper: Sweeping more than 1 parameter is not supported (no support to define matrices in configuration). %s is configured to sweep %i variables  \n", parameterNamesVariableName.data(), parameterNames.size());
...
void CmdLineParameterSweeper::replaceParameterValue(const std::string& parameterName, const std::string& parameterValue){
