...
#define PYTHON_READING_BACKEND_OPTION_VALUE "Python"
#define SCILAB_2_CMDLINE_READING_BACKEND_OPTION_VALUE "Scilab2CmdLine"
...
#define NULL_LOGGING_BACKEND_OPTION_VALUE "NullLogger"
#define CSV_LOGGING_BACKEND_OPTION_VALUE "csv"
#define HDF5_LOGGING_BACKEND_OPTION_VALUE "hdf5"
...
#include "sinks/Loggers/NullLogger.h"
#include "sinks/Loggers/CsvLogger.h"
#include "sinks/Loggers/HDF5Logger.h"
#include "sinks/ParameterReader/IParameterReader.h"
#include "sinks/ParameterReader/Scilab2CmdLineParameterReader.h"
#include "simulator.h"
...
#include "hybrid/DistributionParameters/UniformDistributionParameter.h"      // included also in 'CPP:' section of SimulationExperimentTracker
#include "hybrid/DistributionParameters/PoissonDistributionParameter.h"      // included also in 'CPP:' section of SimulationExperimentTracker
#include "hybrid/DistributionParameters/LognormalDistributionParameter.h"      // included also in 'CPP:' section of SimulationExperimentTracker
#include "hybrid/DistributionParameters/GammaDistributionParameter.h"      // included also in 'CPP:' section of SimulationExperimentTracker
...
enum DISTRIBUTION_PARAMETER { CONSTANT, EXPONENTIAL, PARETO, SPLIT, NORMAL, UNIFORM, POISSON, LOGNORMAL, GAMMA};
std::shared_ptr<IDistributionParameter> readDistributionParameter(const std::string& paramName);

bool readScopedVariable(/*const*/ Simulator* model, const std::string& variableName, int& value);
int readDebugLevel(/*const*/ Simulator* model);
int readLogLevel(/*const*/ Simulator* model, const std::string& variableName);

template<typename T>
T readDefaultParameterValue(const std::string& paramName);
 ...
 */
template<typename T>
bool inner_getCmdLineParam(const std::string& paramName, T& value){
	bool found = false;
	...
	try{
		...
		if(vm.count(paramName) > 0)
		{
			value = vm[paramName].as<T>();
			found = true;
		}
	} catch(std::exception& e) {
		printLog(LOG_LEVEL_DEBUG, "inner_getCmdLineParam: Parsing '%s' throw exception NOT FOUND\n", paramName.data());
	}

	if(!found && OptionManager::has(paramName))
	{
		value = OptionManager::get<T>(paramName);
		found = true;
	}
 ...
 */
template<typename T>
T inner_getCmdLineParam(const std::string& paramName){
	T value;
	...
	printLog(LOG_LEVEL_DEBUG, "Error Parsing CmdLine parameters: parameter '%s' NOT FOUND\n", paramName.data());
	throw std::runtime_error(std::string("Error Parsing CmdLine parameters: parameter '") + paramName + std::string("' NOT FOUND"));
}
...
struct S {
	 ...
	 */
	static T getCmdLineParam(const std::string& paramName, bool2type<false>){
		//			printLog(LOG_LEVEL_FULL_LOGGING, "getCmdLineParam (primary template). Parameter: %s --\n", paramName.data());
		//std::cout <<  "T getCmdLineParam<false> (primary template). Parameter:" << paramName << std::endl;
	 ...
	 */
	static bool getCmdLineParam(const std::string& paramName, T& value, bool2type<false>){
		//std::cout <<  "bool getCmdLineParam<false> (primary template). Parameter:" << paramName << std::endl;
	 ...
	 * Throws exception if parameter not present
	 */
	static T getCmdLineParam(const std::string& paramName, bool2type<true> type){
		//			printLog(LOG_LEVEL_FULL_LOGGING, "getCmdLineParam (numeric specialization). Parameter: %s \n", paramName.data());
		//std::cout <<  "T getCmdLineParam<true> (primary template). Parameter:" << paramName << std::endl;

		T value;
		if(getCmdLineParam(paramName, value, type)){
			return value;
		}
	 ...
	 */
	static bool getCmdLineParam(const std::string& paramName,T& value, bool2type<true>){
		printLog(LOG_LEVEL_FULL_LOGGING, "getCmdLineParam (numeric specialization) - no exceptions. Parameter: %s \n", paramName.data());
		//std::cout <<  "bool getCmdLineParam<true> (numeric specialization). Parameter:" << paramName << std::endl;
		// first try to convert the parameter to the value directly
		// NOTE: not needed any more as already handled by check_conversion_to
		try
		{
		...
		{
			//std::cout << "[ParameterReader::getCmdLineParam] Attempting conversion of: '" << paramName << "'" << std::endl;

			// Last conversion attempt.
			if(check_conversion_to<T>(paramName)){
				value = to<T>(paramName);
				//std::cout << "[ParameterReader::getCmdLineParam] Succesfull convertion: '" << paramName << "'" << std::endl;
				return true;
			}

			//std::cout << "[ParameterReader::getCmdLineParam] Conversion failded. Attempting retrieval from parameters: '" << paramName << "'" << std::endl;
...
struct S <std::vector<std::string>> {
	static std::vector<std::string> getCmdLineParam(const std::string& paramName, bool2type<false>){
		printLog(LOG_LEVEL_FULL_LOGGING, "getCmdLineParam vector<std::string>. Parameter: %s \n", paramName.data());
		//std::cout <<  "std::vector<std::string> getCmdLineParam<std::vector<std::string>> (vector of strings). Parameter:" << paramName << std::endl;

		std::vector<std::string> value;
		if(::detail::inner_getCmdLineParam<std::vector<std::string>>(paramName, value)){
			return value;
		} else {
			throw std::runtime_error("ERROR: Parameter '" + paramName + "' not found in Cmdline configuration. Expecting a std::vector<std::string>");
	...
	static bool getCmdLineParam(const std::string& paramName, std::vector<std::string>& value, bool2type<false>){
		return ::detail::inner_getCmdLineParam<std::vector<std::string>>(paramName, value);
	}
};
 ...
 * NOTE: not needed any more, as check_conversion_to<T> already does this job
 */
template<>
struct S <std::vector<double>> {
	static std::vector<double> getCmdLineParam(const std::string& paramName, bool2type<false>){
		printLog(LOG_LEVEL_FULL_LOGGING, "getCmdLineParam vector<double>. Parameter: %s \n", paramName.data());
		//std::cout <<  "std::vector<double> getCmdLineParam<std::vector<double>> (vectors of double). Parameter:" << paramName << std::endl;
		...
		try
		{
			...
			printLog(LOG_LEVEL_DEBUG, "Parameter not found. Returning a vector with the parameter as the only element \n");
			//std::cout <<  "Parameter converted by lexical cast:" << paramName << std::endl;

			return vec;
		}
		catch(const boost::bad_lexical_cast &)
		{
			//std::cout << "[std::vector<double> getCmdLineParam<std::vector<double>> (vectors of double)] Lexical cast failed. Attempt with check_conversion_to: '" << paramName << "'" << std::endl;

			// Last conversion attempt.
			if(check_conversion_to<std::vector<double>>(paramName)){
				vec = to<std::vector<double>>(paramName);

				//std::cout << "[std::vector<double> getCmdLineParam<std::vector<double>> (vectors of double)] Succesfull convertion: '" << paramName << "'" << std::endl;

				return vec;
			}

			//std::cout << "[std::vector<double> getCmdLineParam<std::vector<double>> (vectors of double)] Conversion failded. Attempting retrieval from parameters: '" << paramName << "'" << std::endl;
 ...
 */
template<typename T>
T getCmdLineParam(const std::string& paramName) {
 ...
 */
template<typename T>
bool getCmdLineParam(const std::string& paramName, T& value) {
	//	printLog(LOG_LEVEL_IMPORTANT,"reading %s from cmdline. \n", paramName.data());
	// call the functions in the detail namespace. Using numeric_limits to check if T is numeric
 ...
 */
template<typename T>
T readDefaultScilabParameterValue(const std::string& paramName, T*){
 ...
 */
template<typename T>
std::vector<T> readDefaultScilabParameterValue(const std::string& paramName, std::vector<T> *){
	...
	if(rows != 1) {
		printLog(LOG_LEVEL_ERROR,"Parameter '%s': more than 1 row defined. Reading matrices from scilab not supported. You must Implement getScialbMatrix in globals.h", paramName.data());
		throw std::runtime_error(paramName + ": more than 1 row defined. Reading matrices from scilab not supported. You must Implement getScialbMatrix in globals.h");
	}
...
T readDefaultParameterValue(const std::string& paramName){
	...
	}else if (boost::iequals(parameterReadingBackend, SCILAB_2_CMDLINE_READING_BACKEND_OPTION_VALUE)){ // Scilb2CmdLine
		//		std::cout << "[ParameterReader] Reading parameter:" << paramName << std::endl;
		IParameterReader* reader = Scilab2CmdLineParameterReader::getInstance("Scilab2Cmdline.params"); // TODO: do this in getReadingBackend only once, returning IParameterReader. Need to implement Scilab and CmdLine as IParameterReaders first

		return reader->readParameter(paramName, static_cast<T*>(0));
	} else if (boost::iequals(parameterReadingBackend, PYTHON_READING_BACKEND_OPTION_VALUE)){
		return OptionManager::get<T>(paramName);
	}
	else { // Error
 ...
 * */
template<typename T>
bool readScopedVariable(/*const*/ Simulator* model, const std::string& variableName, T& value){
