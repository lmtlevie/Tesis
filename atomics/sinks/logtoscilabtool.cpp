...
aLoggerToScilab::aLoggerToScilab(int level, int mID /* currently not used */, const std::string& mName, bool useMultipleSimulationRuns) { // Constructor
...
void aLoggerToScilab::initSignals(const std::map<std::string,int> &variableNamesAndPriorities){
...
void aLoggerToScilab::initSignals(const std::vector<std::string> &variableNames) {
	...
	for (int i = 1; i <= variableNames.size(); i++) {
		...
		signalInfo mySignalInfo = { i, 0, fvar.data() }; // Inicializo a 0
...
void aLoggerToScilab::logSignal(double t, double signalValue, const std::string& signalName) { // Logger
	if (SCILAB_LOG_LEVEL < this->logLevel) { return; }
...
void aLoggerToScilab::flush(double t) { // Flush
