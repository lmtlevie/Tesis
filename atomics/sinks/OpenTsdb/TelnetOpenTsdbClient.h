...
//#include <boost/lambda/lambda.hpp> FAILING COMPILE :-(
...
private:
    ...
    void start_connect();
    void handle_write(const std::string& writtenPoint, const boost::system::error_code & error);
    void handle_write_error(const std::string& error);
    void enqueueWrite(const std::string& addPointCommand);
    void do_close();
    void forceClose();
