...
class aLoggerToScilab : public IPowerDEVSLogger {
	...
	public:
	    ...
	    aLoggerToScilab(int level, int modelId,const std::string& modelName) : aLoggerToScilab(level, modelId,modelName, true) {};
		aLoggerToScilab(int level, int modelId,const std::string& modelName, bool useMultipleSimulationRuns);
		virtual void initSignals(const std::vector<std::string> &variableNames);
		virtual void initSignals(const std::map<std::string,int> &variableNamesAndPriorities);
		virtual void logSignal(double, double, const std::string&);
		virtual void flush(double t);
