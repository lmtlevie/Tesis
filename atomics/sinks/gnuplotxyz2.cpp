...
#include "gnuplotxyz2.h"
#include "sinks/ParameterReader.h"

void gnuplotxyz2::init(double t,...) {
	...
	// open csv file, write and close file
	long int fd = PDFileOpen(FName,'w');
	PDFileWrite(fd,"\n",1);
	PDFileClose(fd);

	n = 3; // formatting lines on graphical interface
	hasOutput = 0;
	char buff2[1096];
...
double gnuplotxyz2::ta(double t) {
	//This function returns a double.
	return sigma;
}

void gnuplotxyz2::dint(double t) {
	sigma=1e20;
}

void gnuplotxyz2::dext(Event x, double t) {
...
Event gnuplotxyz2::lambda(double t) {
	return Event();
}

void gnuplotxyz2::exit() {
	PDFileClose(foutput);
	char Script[128];
	char buff[128];
