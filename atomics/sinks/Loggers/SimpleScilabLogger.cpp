#include "SimpleScilabLogger.h"

int SimpleScilabLogger::TotalFlushingTime_ms = 0;
int SimpleScilabLogger::TotalWrittingTime_us = 0;
int SimpleScilabLogger::TotalMetricPoints = 0;
int SimpleScilabLogger::TotalLoggedVariables = 0;
int SimpleScilabLogger::TotalLoggedModels = 0;

SimpleScilabLogger::SimpleScilabLogger(Simulator* model, bool useMultipleSimulationRuns) : SimpleScilabLogger(model, model->getFullName(), useMultipleSimulationRuns){

}

SimpleScilabLogger::SimpleScilabLogger(Simulator* model, const std::string& logingName, bool useMultipleSimulationRuns) :
	...
	printLog(LOG_LEVEL_DEBUG, "Creating SimpleScilabLogger for model %s \n", logingName.data());
}

/**
 * Initializes all variables without checking the logLevel
 */
void SimpleScilabLogger::initSignals(const std::vector<std::string>& variableNames) {
...
void SimpleScilabLogger::initSignals(const std::map<std::string,int>& variableNamesAndPriorities){
...
void SimpleScilabLogger::initSignals_internal(const std::vector<std::string>& variableNames) {
	SimpleScilabLogger::TotalLoggedVariables += variableNames.size() + 1;
	SimpleScilabLogger::TotalLoggedModels ++;
	...
	for (auto varName : variableNames) {
		...
		printLog(1,"SimpleScilabLogger: Using file %s for model.variable=%s.%s\n", fileName, this->logingName.data(), varName.data());

		this->signalNames[varName] = std::make_shared<SignalInfo>(varName, fileName);
	}
}

void SimpleScilabLogger::logSignal(double t, double signalValue, const std::string& signalName) { // Logger
	auto initWrittingTime = std::chrono::system_clock::now();

	if(!this->signalNames.count(signalName)){
		printLog(LOG_LEVEL_FULL_LOGGING ,"SimpleScilabLogger for model '%s': received var '%s' which was not initialized. ignoring.. \n", this->logingName.data(), signalName.data());
		return;
	}
	auto info = this->signalNames[signalName];

	printLog(LOG_LEVEL_FULL_LOGGING,"SimpleScilabLogger for model '%s': received var '%s' with value=%f and timestamp %f \n", this->logingName.data(), signalName.data(), signalValue, t);
	...
	SimpleScilabLogger::TotalWrittingTime_us += writtingTime_us;
	this->modelWrittingTime_us += writtingTime_us;

	this->modelMetricPoints++;
	SimpleScilabLogger::TotalMetricPoints++;
}

void SimpleScilabLogger::flush(double t) { // Flush
	...
	for(auto search: this->signalNames){
		...
		printLog(LOG_LEVEL_DEBUG,"SimpleScilabLogger::flush - starting to log for %s.%s \n", this->logingName.data(), signalName.data());
		...
		printLog(LOG_LEVEL_DEBUG,"SimpleScilabLogger::flush - info->fileName=%s \n", info->fileName);
		int file = this->openFile(info->fileName);
	...
	SimpleScilabLogger::TotalFlushingTime_ms += flushingTime_ms;

	printLog(LOG_LEVEL_PRIORITY,"[SimpleScilabLogger]: model %s flushing time: %i (ms) \n ", this->logingName.data(), flushingTime_ms);
	printLog(LOG_LEVEL_PRIORITY,"[SimpleScilabLogger]: model %s writing time: %i (us) \n ", this->logingName.data(), this->modelWrittingTime_us);
	printLog(LOG_LEVEL_PRIORITY,"[SimpleScilabLogger]: model %s logged metric points (not including T) : %i  \n ", this->logingName.data(), this->modelMetricPoints);
}

int SimpleScilabLogger::openFile(const char* fileName) {
	int file = -1;
	if (getOs() == WINDOWS)
...
const char* SimpleScilabLogger::createFile() {
...
void SimpleScilabLogger::deleteFile(char* fileName) {
	struct stat info;

	//printLog(LOG_LEVEL_DEBUG,"SimpleScilabLogger: deleting file %s \n",fileName);

	stat(fileName, &info);
	if (S_ISREG(info.st_mode) == 1) { // existe el archivo de datos, es necesario borrarlo.
		if (chmod(fileName, S_IRWXU) != 0)
			printLog(LOG_LEVEL_ERROR,"SimpleScilabLogger: CHMOD Error: %s\n", strerror(errno));
		else {
			stat(fileName, &info);
			//printLog(10,"SimpleScilabLogger: CHMOD permissions are: %08x\n", info.st_mode);
		}

		if (remove(fileName) != 0)
			printLog(LOG_LEVEL_ERROR,"SimpleScilabLogger: REMOVE %s Error: %s\n", fileName, strerror(errno)); //Da error cuando el File tiene parametro de Solo Lectura
		else
			printLog(LOG_LEVEL_FULL_LOGGING,"SimpleScilabLogger: REMOVE %s Successful!\n", fileName);
	}
}
