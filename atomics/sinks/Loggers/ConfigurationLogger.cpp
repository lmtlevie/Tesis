...
std::shared_ptr<IPowerDEVSLogger> ConfigurationLogger::getLogger(int loggerId, const std::string& varName){
	...
	switch (loggerType) {
	case ALL: {
		logger = createDefaultLogger(this->model, fullName);
		break;
	}
	case SAMPLER: {
		auto baseLogger = createDefaultLogger(this->model, fullName);
		logger = std::shared_ptr<IPowerDEVSLogger>(new SamplerLogger(this->model, baseLogger));
		break;
	}case DISCRETE_SAMPLER: {
		auto baseLogger = createDefaultLogger(this->model, fullName);
		logger = std::shared_ptr<IPowerDEVSLogger>(new DiscreteSamplerLogger(this->model, baseLogger));
		break;
	}
	default: {
...
void ConfigurationLogger::createLoggerForEachVariable(const std::vector<std::string>& variableNames) {
...
// TODO: hierarchical model configuration will not work if initialized in the init function
void ConfigurationLogger::initSignals(const std::map<std::string,int>& variableNamesAndPriorities){
...
//		std::cout << "Variable '" << this->model->getFullName() << "::" << var.first <<"': Configured level: " << configuredLogLevel << " >?   setPriority:" << var.second << "\n";
	...
	this->createLoggerForEachVariable(variableNames);
}

// Assumes priority=LOG_LEVEL_IMPORTANT
void ConfigurationLogger::initSignals(const std::vector<std::string>& variableNames) {
	std::map<std::string,int> variableNamesAndPriorities;
	for (auto const & name : variableNames){
		variableNamesAndPriorities[name] = LOG_LEVEL_IMPORTANT;
	}

	this->initSignals(variableNamesAndPriorities);
}

void ConfigurationLogger::logSignal(double t, double signalValue, const std::string& signalName) {
	if(this->variableLogger.find(signalName) == this->variableLogger.end()) return; // signal has no logger associated (not enough priority)
