...
expre = fvar;
debugMsg(LOG_LEVEL_INIT, "[INIT] %s - qss_nlfunction_step::init: expression: %s  \n", this->getFullName().data(),  this->expre);

fvar= va_arg(parameters,char*);
n = (int) readDefaultParameterValue<double>(fvar);
debugMsg(LOG_LEVEL_INIT, "[INIT] %s - qss_nlfunction_step::init: n: %i  \n", this->getFullName().data(),  this->n);

fvar= va_arg(parameters,char*);
purely_static=strcmp(fvar,"Purely static")==0;
debugMsg(LOG_LEVEL_INIT, "[INIT] %s - qss_nlfunction_step::init: Purely static: %s  \n", this->getFullName().data(),  this->purely_static ? "true" : "false");

fvar= va_arg(parameters,char*);
dQmin=readDefaultParameterValue<double>(fvar);
debugMsg(LOG_LEVEL_INIT, "[INIT] %s - qss_nlfunction_step::init: dQmin: %f  \n", this->getFullName().data(),  this->dQmin);

fvar= va_arg(parameters,char*);
dQrel=readDefaultParameterValue<double>(fvar);
debugMsg(LOG_LEVEL_INIT, "[INIT] %s - qss_nlfunction_step::init: dQrel: %f  \n", this->getFullName().data(),  this->dQrel);
...
if (tol<dQmin)
	tol=dQmin;
debugMsg(LOG_LEVEL_DEBUG, "[%g]%s qss_nlfunction_step::int - 2dt=%g -dt=%g f0=%g dt=%g 2dt=%g\n", t, this->getFullName().data(), f_2dt,f_dt,f0,fdt,f2dt);
double eps=1e-20;
if (fabs(y_n1)>eps) {
	sigma=.8* pow(fabs(tol/y_n1),1.0/order);
	debugMsg(LOG_LEVEL_DEBUG, "[%g]%s qss_nlfunction_step::int - With first coefficient sigma=%g y_n1=%g\n",t, this->getFullName().data(), sigma,y_n1);
}   
if (fabs(y_n2)>eps) { 
	debugMsg(LOG_LEVEL_DEBUG, "[%g]%s qss_nlfunction_step::int - With second coefficient sigma=%g y_n2=%g tol=%g\n",t, this->getFullName().data(), .8*pow(fabs(tol/y_n2),1.0/(order+1)),y_n2,tol);
...
while (fabs(sigma)>eps && err>tol)
{
  ...
  debugMsg(LOG_LEVEL_DEBUG, "[t=%g] With iteration sigma=%g\n",sigma);
}
...
if (fabs(sigma)<eps || err>tol) { 
   ...
   while (fabs(pop->Val()-evaluate_poly(y,2*sigma,order))<tol && iter<max_iter) {
    ...
    debugMsg(LOG_LEVEL_DEBUG, "[%g]%s: Using a bisection step sigma=%g (after %d iterations) \n",t, this->getFullName().data(), sigma, iter);
  }


debugMsg(LOG_LEVEL_DEBUG, "[t=%g] sigma=%g\n",t,sigma);
}
...
if (fabs(f[0]-y[0])>tol && sigma)
  debugMsg(LOG_LEVEL_DEBUG, "[t=%g] Diff=%g (tol=%g)\n",t,f[0]-y[0],tol);
