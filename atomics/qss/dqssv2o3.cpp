...
//printLog("%g ENTER init \n", t, this->getFullName()->data());     // DEBUG
//The 'parameters' variable contains the parameters transferred from the editor.
va_list parameters;
va_start(parameters,t);

char *fvar=va_arg(parameters,char*);
double InitialDelay=readDefaultParameterValue<double>(fvar);		// Initial Delay parameter
...
//       		   printLog("Constant History \n");	// DEBUG
	...
	debugMsg(LOG_LEVEL_DEBUG, "[%g] %s - dqssv2o3::init - Constant History.  Bu[%i][0]=%f ; Bt[%i]=%f  \n", t, this->getFullName().data(), PNo, Bu[PNo][0], PNo, Bt[PNo]);	// DEBUG
...
//     'x.port' is the port number
	...
	debugMsg(LOG_LEVEL_DEBUG, "[%g] %s - dqssv2o3::dext - port %i=[%g,%g,%g,%g] \n", t, this->getFullName().data(), x.port, xv[0], xv[1], xv[2], xv[3]);

if (x.port==1) { /////////// New Y Arrived
     
		this->logger->logSignal(t, xv[0], "in_y");
        debugMsg(LOG_LEVEL_DEBUG, "[%g] %s - dqssv2o3::dext -  New Y Arrived: [%g,%g,%g,%g]     ; Tau=[%g,%g,%g,%g] \n",
        		t, this->getFullName().data(), xv[0],xv[1],xv[2],xv[3], Tau[0],Tau[1],Tau[2],Tau[3]);     // DEBUG
		...
		if(PNi==(PNo+1)%MaxQ) {      // segment currently at the output (last emitted,
			...
			debugMsg(LOG_LEVEL_DEBUG, "[%g] %s - dqssv2o3::ext - now (advanced)  Tau: [%g,%g,%g,%g] ;   ty_Fwd=%g  \n", t, this->getFullName().data(), Tau[0],Tau[1],Tau[2],Tau[3], ty_Fwd);     // DEBUG
               ...
               if (zone_Tau=='f')  {	 // The delay was moving FORWARDS in time.
			 			if (nxt=='t')	{      // Next event is a planned zone change for Tau.
								  			   ...
								  			   if (Tau_now<sigma-e) {sigma=Tau_now;    // The new segment y must be treated next, before the Tau changes
						  	                                 ...
						  	                                 };  // The planned change in Tau must be treated next
						          };
						if (nxt=='y')	{      // There is a planned change in y
					  		     printLog("%g dqssv2o3 ERROR: Conflict at the head of the queue \n",t); // ... this shouldn't happend in this special case !
							this->sigma = std::max(0.0, this->sigma - this->e); // continue as before
					  		     };							 
						if (nxt=='i')	{       // There is no planned change
								 //printLog("%g Entered in nxt=='i'  \n",t);     // DEBUG

								 sigma=Tau_now; // The new segment Y must be treated next
								 phase=4;       // Now Y will expire first

								 // MATIAS: new calculation of sigma (to fix bugs with step-wise input can linear tau)
								 // we need tf such that t=tf-Tau(tf)
								 //   => tf = http://www.wolframalpha.com/input/?i=solve(x-+(%5B%2F%2Fnumber:c%2F%2F%5D+*+x%5E2+%2B+%5B%2F%2Fnumber:b%2F%2F%5D+*+x+%2B+%5B%2F%2Fnumber:a%2F%2F%5D)++%3D+%5B%2F%2Fnumber:t%2F%2F%5D)
								 if(t!=0){
									 double tf=0;
									 if(Tau[2]==0){
										 // NOTE: b!=1, because that would be the case of zone_Tau=='s'
										 tf = Tau[0] + t / (1 - Tau[1]); // a+t/ (1-b)
									 } else{
										 double sqr = sqrt(-4*Tau[2]*Tau[0] + pow(Tau[1], 2) - 2*Tau[1] - 4*Tau[2]*t + 1 );
										 tf = (sqr + Tau[1] - 1) / (2*Tau[2]);
									 }
									 if(tf>=t) sigma=tf-t;
		       ...
		       if (zone_Tau=='b') {		  // The delay was moving BACKWARDS in time.
		    	   this->sigma = std::max(0.0, this->sigma - this->e); // continue as before        // Resume previous wait, whatever it was (for Tau or for Y)
  					       };
		...
		// No special case
			this->sigma = std::max(0.0, this->sigma - this->e); // continue as before		  // Resume previous wait.
		};
		//printLog("%g now  Tau: [%g,%g,%g,%g] \n",t,Tau[0],Tau[1],Tau[2],Tau[3]);     // DEBUG
		if((PNi+1) != (PNi+1)%MaxQ){
			printLog("[%g] %s - dqssv2o3::ext - History queue exceeded, from now on it will replace old values. PNi=%l ;  MaxQ=%l ; (PNi+1)%MaxQ=%l \n", t, this->getFullName().data(), PNi, MaxQ, (PNi+1)%MaxQ);
		}
		PNi=(PNi+1)%MaxQ;


		// MATIAS: emulate the arrival of a new tau (another option to fix bugs with step-wise input can linear tau)
//		advance_time(Tau, e, -1);
...
if (x.port==0) { ///////// New Tau Arrived
	...
	debugMsg(LOG_LEVEL_DEBUG,  "[%g] %s - dqssv2o3::ext - New Tau Arrived: [%g,%g,%g,%g] \n", t, this->getFullName().data(), xv[0], xv[1], xv[2], xv[3]);     // DEBUG
	...
	//printLog("%g Leaving dext. Sigma=%g  Tau: [%g,%g,%g,%g] \n",t, sigma, Tau[0],Tau[1],Tau[2],Tau[3]);     // DEBUG
}
...
long 	PNt=-1 ;  		 // Target Pointer to be used as the new value for PNo
   ...
   switch (phase) {
	  case 1:			// New Tau segment arrived
		  ...
		  if (Tau[2]==0 && Tau[3]==0) { 	// At most linear evolution of Tau, wont change its direction
 			      ttau_Inv=INF;
		  ...
		  if (PNt<0) {     // CONSISTENCY CHECK
		   	 		  printLog("%g dqssv2o3 ERROR: FAILED to find new PNt or PNt less than 0 \n",t);
          ...
          if (ty_Bck<0) {      printLog("%g dqssv2o3 ERROR: ty_Bck<0 -> Force ty_Bck=0 \n",t);
	      ...
	      if (ty_Fwd<0) {      printLog("%g  dqssv2o3 ERROR: ty_Fwd<0 -> Force ty_Fwd=0 \n",t);
	  ...
	  case 3:			// Tau segment expired
	      ...
	      if (ty_Fwd<0) {       printLog("%g dqssv2o3 ERROR: ty_Fwd<0 (%g) -> Force ty_Fwd=0 \n",t,ty_Fwd);
		  	 		  ty_Fwd=0;}; // CONSISTENCY CHECK
	  ...
	  //#######// Recalc. invertion time of Tau direction (Backwards<->Forward): ttau_Inv
		  if (Tau[2]==0 && Tau[3]==0) {ttau_Inv=INF;	            // At most linear evolution of Tau, wont change its direction
	  ...
	  //#######// Calculates new PNt (Increment/Decrement PNo)
       ...
       if (zone_Tau=='f') {	// The delay was MOVING FORWARDS in time, and reached the beggining of the NEXT Y segment
              ...
              if (PNt>PNi-1) {      printLog("%g dqssv2o3 ERROR: PNt>PNi-1 -> Force PNt=PNi-1 \n",t);
 		  	 			            printLog("%g dqssv2o3 ERROR: PNt=%d , PNi=%d , PNo=%d \n",t,PNt,PNi,PNo);
       ...
       if (zone_Tau=='b') {		  // The delay was MOVING BACKWARDS in time, and reached the beggining of the CURRENT Y segment
              ...
              if (PNt<0)  {        printLog("%g dqssv2o3 ERROR: PNt<0  -> Force PNt=0  \n",t);
       ...
       if (zone_Tau=='s') {		  // The delay was remaining FIXED in time, targeting a stationary point.
       	  				  // CONSISTENCY CHECK
					      printLog("dqssv2o3 ERROR: Y changed with Tau Stationary");     // THIS SHOULDN'T HAPPEND HERE ! If Tau is stationary, a Y change is never scheduled
			 };  			  
		  break;


       if (PNt<0) {   printLog("%g dqssv2o3 ERROR: FAILED to find new PNt or PNt less than 0 \n",t);
	  ...
	  if (PNt<0) {  // CONSISTENCY CHECK
	  	 		      printLog("%g dqssv2o3 ERROR: FAILED to find new PNt or PNt less than 0. Force PNt=PNo. \n",t);
				   PNt=PNo;}; // ERROR, Make PNt=PNo (segment unchanged)
	  ...
	  switch (nxt) { 
		  ...
		  case 'i': phase=5;		// (Infinite: Phase were no expiration is expected)
		  	   break;
		  default:      			printLog("%g dqssv2o3 ERROR: INVALID nxt value:%g \n",t,nxt); // ERROR;
               break;
			 };


      //#######// y[] is already properly calculated
	  this->logger->logSignal(t, y[0], "out");
	  debugMsg(LOG_LEVEL_IMPORTANT, "[%g] %s - dqssv2o3::lambda  - output: [%g,%g,%g,%g] \n", t, this->getFullName().data(), y[0],y[1],y[2],y[3]);     // DEBUG
      return Event(y,0);
}
//void dqssv2o3::exit() {
//
