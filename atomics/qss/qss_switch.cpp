...
if (x.port==1){
	...
	if (((u[1][0]>0)&&(sw==2))||((u[1][0]<=0)&&(sw==0))){ //we migth be in the wrong position
		...
		sigma=0;
		change=1; //we plan to switch in the next event at 0
	} else if(sigma != 0) {  // bug fix (matias): to prevent loosing scheduled outputs. see: https://sourceforge.net/p/powerdevs/discussion/general/thread/8072e4d6/
		sigma=tcross;
		change=1; //we plan to switch in the next event at tcross
	}
} else {
	tcross=tcross-e; 
...
//if (sigma>0)advance_time(y,sigma,-1); // bug fix (matias): to prevent advancing time twice, already done in lines 95-98. See: https://sourceforge.net/p/powerdevs/discussion/general/thread/8072e4d6/
