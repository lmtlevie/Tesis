...
//  int result = -1;
  ...
  if (write(SendSocket,buff,strlen(buff))<(signed)strlen(buff)){
    printLog("Incomplete TCP message. Buffer was:\n");
  	printLog("'%s' \n Command:", buff);
  	printLog("'%s' \n", cmd);
  	throw std::runtime_error("Error reading from Scilab: Incomplete TCP message. See log for details.");
  }if (blocking) {
...
void getScilabMatrix(const char* varname, int *rows, int *cols, double **data) {
  ...
  executeVoidScilabJob(buf,true);
  sprintf(buf,"save('%s/../output/temp.dat',tempvar)",getExecutablePath().c_str());
  executeVoidScilabJob(buf,true);
  FILE *FOpen;
...
void getScilabVector(const char* varname, int *length, double *data) {
  ...
  int rows,cols;
  char buf[1024];
  sprintf(buf,"clear tempvar");
  executeVoidScilabJob(buf,true);
  sprintf(buf,"tempvar=%s",varname); // varname can actually be a command (ej: size(myvariable)) so we need to assign the result to a variable first
  executeVoidScilabJob(buf,true);
  sprintf(buf,"save('%s/../output/temp.dat',tempvar)",getExecutablePath().c_str());
  executeVoidScilabJob(buf,true);
...
// moved to RootSimulator::FinalTime
//double getFinalTime()
//{
//	return tf;
//}
