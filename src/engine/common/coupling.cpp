...
double Coupling::ta(Time t){
  if (Dsize==0) return 10e20;// No childs

	transitionChild=heap.next();
	tn=D[transitionChild]->tn;

//	if (tn-t < 0){
//		printLog("[%g] negative ta=%f (tn=%f, t=%f) in child model %s (father: %s) \n", t, tn-t, tn, t, D[transitionChild]->name, name);
//		throw std::runtime_error("["+ std::to_string(t) + "]  negative ta="+std::to_string(tn-t) + " in child model " + this->getFullName() +"."+std::string(D[transitionChild]->name)  );
//	}
