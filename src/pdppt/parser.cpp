...
modelCoupled *parseModel(QString filename)
{
	...
	std::string path_str, filename_str = filename.toUtf8().constData();
	ssize_t index = filename_str.find_last_of("/");

	if(index == std::string::npos)
		path_str = ".";
	else
		path_str = filename_str.substr(0, index);

	path = QString::fromStdString(path_str);

	fd = new QFile(filename);
	if (!(filename.endsWith(".pdm", Qt::CaseInsensitive)) && !(filename.endsWith(".pdl", Qt::CaseInsensitive))) {
		printf("File must be either a .pdm or .pds or .pdl\n");
		exit(-1);
	}
