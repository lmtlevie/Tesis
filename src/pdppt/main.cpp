...
{
	bool useGUI = !qgetenv("DISPLAY").isEmpty();
	...
	if (args.size()==1) {
		...
		printf("	           -p Generate python library models\n");
		printf("	           -c Do not compile\n");
		printf("	           -f file Use the supplied Makefile\n");
		printf("	           -pdif binary Use the supplied interface for running\n");
		printf("	           -path <path> Look for the missing models in this path\n");
	...
	for (int i=1;i<args.size();i++) {
		...
		} else if (args.at(i)=="-path") {
			if (i+1<args.size())
				user_path=args.at(++i);
		} else { // pdm or pdf filename
			filename = args.at(i);
		}
	}
	...
	// if python option generate and finish
	modelCoupled *c = NULL;
	if(python){
		if(!filename.endsWith(".pdm", Qt::CaseInsensitive) && !filename.endsWith(".pdl", Qt::CaseInsensitive)){
			printf("Only .pdm or .pdl files allowed for python code generation \n");
			return -1;
		}

		// parse file
		c = parseModel(filename);
		QString pdsFile = filename.replace(".pdm", ".pds", Qt::CaseInsensitive).replace(".pdl", ".pds", Qt::CaseInsensitive);
		generateCode(c, pdsFile, false, true, true, user_path); // this generates the .pds which is needed to fill and compute the connetions

		// generate python code
		QString outfile = QFileInfo(filename).baseName() + ".py";
		auto generator = PythonCodeGenerator(c, outfile);
		generator.generatePythonLibrary();

		return 0; // finish here
	}

	// parse .pdm, .pds and generate
	if (filename.endsWith(".pdm", Qt::CaseInsensitive)) {
		c = parseModel(filename);
		generateCode(c, filename.replace(".pdm", ".pds", Qt::CaseInsensitive), generateCCode, true, silent, user_path);
	} else if (filename.endsWith(".pds", Qt::CaseInsensitive)) {
		c = parsePDS(filename);
		generateCode(c, filename.replace(".pdm", ".pds", Qt::CaseInsensitive), generateCCode, false, silent, user_path);
	} else {
	...
	// if set not to compile finish here (no compile and no run)
	if(!compile) 
		return 0;
	...
	if (make.exitCode() == 0) {
		printf("Compilation finished successful. \n");
		...
		printf("Error: The compilation process has reported an error. Check 'output/compile.log' for details. \n");
