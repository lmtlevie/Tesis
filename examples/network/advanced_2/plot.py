...
def plot_drops(plotter):

    time1, values1 = plotter.getTimeSerie('Router1/egressPort0/queue/discard_bits')
    time2, values2 = plotter.getTimeSerie('Router1/egressPort0/RED_AQM/drop_bits')
...
def make_plots(plotter):
    ...
    plt.subplot(3, 1, 2)
    plot_qsize_packet(plotter, "Router1", port=0,scale=1e6, withREDAvg=True)
    
    plt.subplot(3, 1, 3)    
    plot_drops_packet(plotter, "Router1", port=0,scale=1e6, withRED=True)

    plt.show()

if __name__ == '__main__':
    h5_file_name = "network_advanced_2_0.h5"    
    if len(sys.argv) > 1:
