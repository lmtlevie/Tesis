...
1. Open the model. Go to File -> Open, the choose folder `continuous/` and the file `nl_stiff.pdm`.
...
```bash
cd powerdevs/
bin/pdppt -m examples/continuous/nl_stiff.pdm
```
...
3. Finally, run the model specifying the final simulation time (in this case 500s):
```bash
cd ../output/
./model -tf 500
```
